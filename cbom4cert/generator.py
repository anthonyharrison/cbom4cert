...
class CBOMGenerator:
    ...
    def check_file(self, filename):
        ...
        if filename is None:
            return False
        if len(filename) > 0:
            # Handle case where file does not exist, in which case stats wouldn't be accessible
            if Path(filename).exists():
                file_exists = Path(filename).stat().st_size > 0
        return file_exists
    ...
    def check_compliance(self, cert, key_info):
        ...
        if cert.signature_hash_algorithm and cert.signature_hash_algorithm.name in [
            "sha1",
            "md5",
        ]:
                    ...
                    for file in files:
                        if file.endswith((".crt", ".pem")):
                            with open(os.path.join(root, file), "rb") as f:
                                certs.append(
                                    self.get_cert_metadata(
                                        f.read(), "PEM", os.path.join(root, file)
                                    )
                                )
        return certs
    ...
    def process_certificate(self, filename):
        if platform.system() not in ["Windows", "Darwin"]:
            if filename.endswith((".crt", ".pem")) and self.check_file(filename):
                with open(filename, "rb") as f:
                    self.inventory.append(
                        self.get_cert_metadata(f.read(), "PEM", filename)
                    )
    ...
    def create_cbom(self, sbom_type="cyclonedx", sbom_format="json", outfile=""):
        ...
        my_generator = SBOMGenerator(
            False, sbom_type=sbom_type, format=sbom_format, version=VERSION
        )
...
if __name__ == "__main__":
    cbomgen = CBOMGenerator()
    cbomgen.get_system_certificates()
    cbomgen.create_cbom()
